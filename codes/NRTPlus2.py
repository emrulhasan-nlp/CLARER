...
import math
import time
from utils import preparedata,create_dataloader,datasplit, test_explanation
...
λ1, λ2, λ3 = 1.5,0.5,1.0   # Weight for explanation loss, # Weight for rating prediction loss, Weight for contrastive loss (typically small)
...
for epoch in range(num_epochs):
    ...
    for batch in train_loader:
        ...
        #output,pred_rating, pred_criteria, aspRep=model(userId, itemId, src_batch, tgt_batch[:, :-1]) #Output: Decoder output for explanation, rating is the predicted ratings, aspect_rep is review rep
        if criteria:
        ...
        for batch in val_loader:
           ...
           # batch_len=len(userId)
            ...
            if criteria:
              ...
              xpln_loss = criterion(output.contiguous().view(-1, tgt_vocab_size), tgt_batch[:, 1:].contiguous().view(-1))
              rating_loss = rating_criterion(pred_rating.squeeze(), ratings.squeeze())
              criteria_loss = rating_criterion(pred_criteria.reshape(-1), criteria_ratings.reshape(-1))
...
start_time=time.time()
output_file=f"{λ1}_criteria{criteria}_aspect{calculate_aspRep}_xpltest.txt"

print(criteria, calculate_aspRep)
test_xplanationPairs=test_explanation(test_data, model, device,criteria,calculate_aspRep, output_file)
